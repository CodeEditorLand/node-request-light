...
import { type Url, format, parse as parseUrl } from "url";
...
export function getErrorStatusDescription(status: number): string {
	...
	switch (status) {
		...
		case 502:
			return localize(
				"status.502",
				"Bad Gateway. The upstream server did not respond.",
			);
		case 503:
			return localize(
				"status.503",
...
function getProxyAgent(
	...
	return requestURL.protocol === "http:"
		? createHttpProxyAgent(opts)
		: createHttpsProxyAgent(opts);
}
